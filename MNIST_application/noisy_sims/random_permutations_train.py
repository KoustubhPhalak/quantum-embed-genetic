'''
Code to select random permutations of input configurations and train them
'''
...
for case, ind_list in enumerate(random_perms):
    ...
    if not os.path.exists(f"{save_dir}/random_perms_{cls1}_{cls2}.txt"):
        with open(f"{save_dir}/random_perms_{cls1}_{cls2}.txt", 'w+') as f:
            lines = f.readlines()
    else: 
        with open(f"{save_dir}/random_perms_{cls1}_{cls2}.txt", 'r') as f:
            lines = f.readlines()
    ...
    if case + 1 < len(random_perms): # First P-1 permutations
        ...
        lines[0] = lines[0].strip() + str(tr_acc_list[-1])+'\n'
        lines[1] = lines[1].strip() + str(inf_acc_list[-1])+'\n'
    with open(f"{save_dir}/random_perms_{cls1}_{cls2}.txt", 'w') as f: # Write lines
        f.writelines(lines)
