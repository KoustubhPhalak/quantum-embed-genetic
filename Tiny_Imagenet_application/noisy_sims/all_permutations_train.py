'''
Code to train all permutations of 3 qubit QNN on the compressed Tiny ImageNet dataset
'''
...
for case, ind_list in enumerate(list(itertools.permutations(indices))):
    ...
    if not os.path.exists(f"{save_dir}/brute_force_hi.txt"):
        with open(f"{save_dir}/brute_force_hi.txt", 'w+') as f:
            lines = f.readlines()
    else: 
        with open(f"{save_dir}/brute_force_hi.txt", 'r') as f:
            lines = f.readlines()
    ...
    if case + 1 < len(list(itertools.permutations(indices))): # First P-1 permutations
        ...
        lines[0] = lines[0].strip() + str(tr_acc_list[-1])+'\n'
        lines[1] = lines[1].strip() + str(inf_acc_list[-1])+'\n'
    with open(f'{save_dir}/brute_force_hi.txt', 'w') as f: # Write lines
        f.writelines(lines)
